...
function Stat({ label, value, sub }: { label: string; value: string; sub?: string }) {
  return (
    <div className="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3">
      ...
      <div className="text-[19px] font-semibold text-slate-900 mono mt-0.5">{value}</div>
...
export function MonthlyDetailTable({ points, isMoney }: { points: ForecastPoint[]; isMoney: boolean }) {
  ...
  return (
    <div>
      ...
      >
        ...
        Monthly forecast detail ({points.length} months)
      </button>
      {open && (
            ...
            <tbody>
              {points.map((p) => (
                <tr key={p.month} className="border-t border-slate-100">
                  ...
                  <td className="px-3 py-2 text-right text-slate-800 font-medium mono">{fmtNumber(p.predicted, isMoney)}</td>
                  <td className="px-3 py-2 text-right text-slate-500 mono">{fmtNumber(p.low, isMoney)}</td>
                  <td className="px-3 py-2 text-right text-slate-500 mono">{fmtNumber(p.high, isMoney)}</td>
                </tr>
              ))}
            </tbody>
          </table>
